...
@app.post("/api/upload")
def upload_exam_materials(
    plan_title: str = Form(...),
    question_bank: UploadFile = File(...),
    db: Session = Depends(get_db),
):
    print(f"[DEBUG] /api/upload: plan_title={plan_title}, filename={question_bank.filename}")

    # Preserve the original extension so the parser can detect the file type
    original_filename = question_bank.filename or "upload"
    ...
    try:
        print("[DEBUG] Parsing file for questions…")
        try:
            extracted_questions = parse_question_pdf(temp_path)
        except ValueError as ve:
            # Surface a clean, user-readable message for unsupported formats
        ...
        if not extracted_questions:
            raise HTTPException(
                ...
                detail=(
                    ...
                    "Please check the format: questions should begin with "
                    "'Question #1' or '1.' followed by lettered options (A. B. C. D.)."
                ),
            )

        print(f"[DEBUG] Extracted {len(extracted_questions)} questions.")
        ...
        db.commit()
        print(f"[DEBUG] Committed all questions. Plan ID: {new_plan.id}")
...
def get_questions(plan_id: str, db: Session = Depends(get_db)):
    return (
        db.query(Question)
        .filter(Question.exam_plan_id == plan_id)
...
def get_progress(plan_id: str, db: Session = Depends(get_db)):
    ...
    # Only fetch attempts belonging to questions in this plan (optimised)
    question_ids = [q.id for q in questions]
    attempts = {
        a.question_id: a
        for a in db.query(UserAttempt)
        .filter(UserAttempt.question_id.in_(question_ids))
        .all()
    }
    progress = []
    for q in questions:
        ...
        progress.append(
            {"question_id": q.id, "question_number": q.question_number, "status": status}
        )
    return progress
...
async def evaluate_answer(payload: AttemptSubmit, db: Session = Depends(get_db)):
    ...
    async def event_generator():
        ...
        is_correct = "INCORRECT" not in first_line.upper()
...
async def ask_followup(payload: ChatSubmit):
    async def event_generator():
        async for text_chunk in stream_chat(
            payload.question_text, payload.ai_explanation, payload.user_message
        ):
            yield f"data: {json.dumps({'text': text_chunk})}\n\n"
