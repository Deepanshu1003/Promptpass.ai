...
def get_progress(plan_id: str, db: Session = Depends(get_db)):
    """Get progress for all questions in an exam plan."""
    questions = db.query(Question).filter(Question.exam_plan_id == plan_id).order_by(Question.question_number).all()
    # Get only attempts for questions in this plan (optimization)
    question_ids = [q.id for q in questions]
    attempts = {a.question_id: a for a in db.query(UserAttempt).filter(UserAttempt.question_id.in_(question_ids)).all()} if question_ids else {}
    
    progress = []
    for q in questions:
        status = "gray"  # Not attempted
        if q.id in attempts:
